...
- 기존 프로젝트와의 중복을 피하기 위해 pms_board2를 사용한다.

```
create table pms_board2(
  no int not null,
...
alter table pms_board2
  add constraint pms_board2_pk primary key(no);
  ...
  public void execute() {
    ...
    try (Connection con = DriverManager.getConnection(
      ...
      String sql = String.format(
          "insert into pms_board2(title,content,writer) values('%s','%s','%s')",
...
public class BoardDeleteCommand implements Command {
  ...
  public void execute() {
    ...
    try (Connection con = DriverManager.getConnection(
      ...
      String sql = String.format("delete from pms_board2 where no=%d", no);
